...
_CONTROL_LINES = {
    "jellyfin_play": (
        ...
        "так і скажи. «Перенеси сесію з X на Y» = візьми з jellyfin_sessions для X item_id і "
        "PlayState.PositionTicks, виклич jellyfin_play на session_id пристрою Y з тим самим item_id і "
        "start_position_ticks = ця позиція (щоб продовжити з того самого місця, не спочатку). Якщо на Y вже "
        "грає ІНШИЙ контент (не той, що переносиш) — скажи, що саме там зараз, і перепитай, чи точно замінити "
        "(PlayNow перезапише без попередження); не відмовляйся мовчки і не переноси без цього уточнення. "
        "X і Y названі прямо користувачем — не плутай, хто джерело, а хто ціль\n"
    ),
